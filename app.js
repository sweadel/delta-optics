...
window.performGlobalSearch = () => { 
    ...
    document.querySelectorAll('.workspace.active table tbody tr').forEach(tr => { 
        tr.style.display = tr.innerText.toUpperCase().includes(f) ? "" : "none"; 
    }); 
};

// ================== Security & Audit ==================
function getDeviceInfo() { const ua = navigator.userAgent; return /Mobile|Android|iP(hone|od)|IEMobile/.test(ua) ? "Mobile" : "Desktop"; }
...
// ================== Authentication (تسجيل الدخول والرتب) ==================
window.handleLogin = async () => {
    ...
    if (res.success) {
        ...
        applyRoles(Auth.user.role); logAudit("تسجيل دخول"); startSync(); window.navigate('ws-dashboard');
    } else { Swal.fire('مرفوض', res.msg, 'error'); }
};
window.handleLogout = () => { logAudit("تسجيل خروج"); Auth.logout(); };

function applyRoles(role) {
    let roleName = "موظف"; document.querySelectorAll('.admin-only, .dev-only').forEach(el => el.style.display = 'none');
    ...
    if (role === 'developer' || role === 'superadmin') { roleName = "مطور النظام"; document.querySelectorAll('.admin-only, .dev-only').forEach(el => el.style.display = 'flex'); }
    if(document.getElementById('display-role')) document.getElementById('display-role').innerText = roleName;
}

document.addEventListener('DOMContentLoaded', () => { if (Auth.check()) { document.getElementById('login-modal').style.display = 'none'; document.getElementById('display-user').innerText = Auth.user.name; applyRoles(Auth.user.role); startSync(); window.navigate('ws-dashboard'); } });

// ================== Soft Delete (المحذوفات) ==================
...
// ================== Financials (الحاسبة الفورية والمصروفات) ==================
...
window.renderDailyLedger = () => {
    ...
    todayInvoicesData.forEach(i => {
        if (i.time?.toDate().toDateString() === todayStr) {
            ...
            combined.push({ timeObj: i.time?.toDate(), timeStr: i.time?.toDate().toLocaleTimeString('en-US', {hour:'2-digit', minute:'2-digit'}) || '--', desc: i.isUnified ? `ملف: ${i.pName}` : `بيع: ${i.pName}`, val: `<span class="en-num" style="color:var(--success); font-weight:800;">+ ${Number(i.paid||0).toFixed(2)}</span>` });
        }
    });
    todayExpensesData.forEach(e => {
        if (e.time?.toDate().toDateString() === todayStr) {
            totalExpenses += Number(e.amount || 0);
            combined.push({ timeObj: e.time?.toDate(), timeStr: e.time?.toDate().toLocaleTimeString('en-US', {hour:'2-digit', minute:'2-digit'}) || '--', desc: `مصروف: ${e.desc} (${e.user})`, val: `<span class="en-num" style="color:var(--danger); font-weight:800;">- ${Number(e.amount||0).toFixed(2)}</span>` });
        }
    });
...
function printUnifiedInvoice(data) {
    ...
    document.getElementById('pr-content').innerHTML = `<div class="print-wrapper"><div class="p-header"><h2>Delta Optics</h2><p>Medical Rx & Receipt | وصفة طبية وفاتورة</p></div><div class="p-info"><span><b>No:</b> <span class="en-num-print">${data.invId}</span></span><span><span class="en-num-print">${dateStr} ${timeStr}</span></span></div><div class="p-info"><span><b>المراجع:</b> ${data.pName}</span><span class="en-num-print">${data.phone || '---'}</span></div><div class="p-title">القياسات (Optical Rx)</div><table class="p-table"><tr><th>Eye</th><th>SPH</th><th>CYL</th><th>AXIS</th><th>ADD</th></tr><tr><th>R (OD)</th><td><span class="en-num-print">${rx.od.s||'-'}</span></td><td><span class="en-num-print">${rx.od.c||'-'}</span></td><td><span class="en-num-print">${rx.od.a||'-'}</span></td><td><span class="en-num-print">${rx.od.add||'-'}</span></td></tr><tr><th>L (OS)</th><td><span class="en-num-print">${rx.os.s||'-'}</span></td><td><span class="en-num-print">${rx.os.c||'-'}</span></td><td><span class="en-num-print">${rx.os.a||'-'}</span></td><td><span class="en-num-print">${rx.os.add||'-'}</span></td></tr></table><div style="font-size:9pt; margin-bottom:15px;"><b>PD:</b> <span class="en-num-print">${rx.pd||'-'}</span> | <b>ملاحظة:</b> ${rx.notes||'-'}</div>${data.subtotal > 0 ? `<div class="p-title">المشتريات (Purchases)</div><table class="p-table"><tr><th>البيان</th><th style="width:70px;">JOD</th></tr>${s.frame.type ? `<tr><td>إطار: ${s.frame.type}</td><td><span class="en-num-print">${s.frame.price.toFixed(2)}</span></td></tr>` : ''}${s.lenses.type ? `<tr><td>عدسات: ${s.lenses.type}</td><td><span class="en-num-print">${s.lenses.price.toFixed(2)}</span></td></tr>` : ''}${s.cl.type ? `<tr><td>لاصق: ${s.cl.type}</td><td><span class="en-num-print">${s.cl.price.toFixed(2)}</span></td></tr>` : ''}${s.extras.type ? `<tr><td>أخرى: ${s.extras.type}</td><td><span class="en-num-print">${s.extras.price.toFixed(2)}</span></td></tr>` : ''}</table><div class="p-summary"><div class="p-row"><span>Subtotal:</span><b class="en-num-print">${data.subtotal.toFixed(2)}</b></div>${data.discountPercent > 0 ? `<div class="p-row"><span>Discount ${data.discountPercent}%:</span><b class="en-num-print">- ${(data.subtotal * (data.discountPercent/100)).toFixed(2)}</b></div>` : ''}<div class="p-row total"><span>Total:</span><b class="en-num-print">${data.total.toFixed(2)}</b></div><div class="p-row"><span>Paid (${pMethod}):</span><b class="en-num-print">${data.paid.toFixed(2)}</b></div><div class="p-row"><span>Due الباقي:</span><b class="en-num-print">${data.due.toFixed(2)}</b></div></div>` : ''}<div style="text-align:center; margin-top:20px; font-weight:600; font-size:8pt;"><p style="margin:0;">بواسطة: ${data.doctor || 'موظف'} | ✨ نتمنى لكم رؤية واضحة ✨</p></div></div>`;
    window.print();
}
...
window.showPatientHistory = (patientName) => {
    ...
    else { records.forEach(r => { tbody.innerHTML += `<tr><td class="en-num">${r.time?.toDate().toLocaleDateString('en-GB') || '--'}</td><td class="en-num">${r.invId}</td><td>${r.prodName}</td><td class="en-num" style="font-weight:bold; color:var(--brand-primary);">${parseFloat(r.total).toFixed(2)}</td><td style="display:flex; gap:5px;"><button class="btn btn-outline" style="padding:6px 12px; font-size:0.9rem;" onclick="printFromData('${r.invId}')"><i class="fas fa-print"></i></button> <button class="btn btn-danger admin-only" style="padding:6px 12px; font-size:0.9rem;" onclick='deleteInvoice("${r.id}", ${JSON.stringify(r).replace(/'/g, "\\'")}, "${r.invId}")'><i class="fas fa-trash"></i></button></td></tr>`; }); }
...
window.printPatientHistory = () => {
    ...
    document.getElementById('pr-content').innerHTML = `<div class="print-wrapper"><div class="p-header"><h2>Delta Optics</h2><p>Patient History | كشف سجل</p></div><div class="p-info"><span><b>المراجع:</b> ${pName}</span><span><span class="en-num-print">${new Date().toLocaleDateString('en-GB')}</span></span></div><table class="p-table"><tr><th>التاريخ</th><th>الملف</th><th>التفاصيل</th><th>JOD</th></tr>${rowsHtml}</table><div class="p-summary"><div class="p-row total"><span>Total Spent:</span><span class="en-num-print">${totalSpent.toFixed(2)}</span></div></div></div>`; window.print(); logAudit(`طباعة كشف: ${pName}`);
};

// ================== POS (المبيعات السريعة) ==================
...
window.createInvoice = async () => {
    ...
    try {
        ...
        logAudit(`بيع كاشير: ${invId}`); Swal.fire({ icon: 'success', title: 'تم الحفظ', timer: 1500, showConfirmButton: false }); printPosFromData(invId, pName, prodName, subtotal, discountPercent, total, paid, due, paymentMethod);
...
window.printPosFromData = (invId, pName, prodName, subtotal, discountPercent, total, paid, due, pMethod) => {
    ...
    document.getElementById('pr-content').innerHTML = `<div class="print-wrapper" style="width:90mm; text-align:center;"><div class="p-header" style="text-align:center;"><h2>Delta Optics</h2><p>Sales Receipt</p></div><div class="p-info" style="font-size:9pt; border-bottom:1px dashed #ccc; padding-bottom:5px;"><span>No: <span class="en-num-print">${d.invId}</span></span><span><span class="en-num-print">${d.time.toLocaleDateString('en-GB')}</span></span></div><div style="text-align:left; font-size:9.5pt; margin-bottom:10px; margin-top:10px;"><b>الزبون:</b> ${d.pName}<br><b>المنتج:</b> ${d.prodName}</div><div class="p-summary" style="text-align:left;"><div class="p-row total"><span>Total:</span> <span class="en-num-print">${d.total.toFixed(2)}</span></div><div class="p-row"><span>Paid:</span> <span class="en-num-print">${d.paid.toFixed(2)}</span></div><div class="p-row"><span>Due:</span> <span class="en-num-print">${d.due.toFixed(2)}</span></div></div><p style="font-size:8pt; margin-top:10px;">✨ شكراً لزيارتكم ✨</p></div>`; window.print();
};
...
window.saveProduct = async () => { const name = document.getElementById('p-name').value, price = document.getElementById('p-price').value, type = document.getElementById('p-type').value, qty = document.getElementById('p-qty').value, img = document.getElementById('p-base64').value; if (!name || !price || !type) return Swal.fire('تنبيه', 'أكمل البيانات', 'warning'); await addDoc(collection(db, "products"), { name, price: Number(price), type, qty: Number(qty), img: img || "", time: serverTimestamp() }); logAudit(`إضافة منتج: ${name}`); Swal.fire('نجاح', 'تم الإضافة', 'success'); document.getElementById('p-name').value = ''; document.getElementById('p-price').value = ''; document.getElementById('p-qty').value = ''; document.getElementById('p-base64').value = ''; };
window.saveExtCollection = async () => { const name = document.getElementById('ext-col-name').value, type = document.getElementById('ext-col-type').value, img = document.getElementById('ext-col-base64').value; if (!name || !img) return Swal.fire('تنبيه', 'إجباري!', 'warning'); await addDoc(collection(db, "brands"), { name, type, imageUrl: img, timestamp: serverTimestamp() }); Swal.fire('تم', 'تم الإضافة', 'success'); logAudit(`إضافة موديل: ${name}`); document.getElementById('ext-col-name').value=""; document.getElementById('ext-col-base64').value=""; };
...
function startSync() {
    ...
    onSnapshot(query(collection(db, "invoices"), orderBy("time", "desc")), (s) => {
        ...
        s.forEach(d => { 
            ...
            if (!i.isUnified) { window.allUnifiedRecords.push(i); tbInvoices += `<tr><td class="en-num">${i.invId}</td><td style="font-weight:700;">${i.pName}</td><td>${i.prodName}</td><td class="en-num" style="font-weight:800; color:var(--brand-primary);">${parseFloat(i.total).toFixed(2)}</td><td class="en-num" style="color:var(--danger); font-weight:800;">${parseFloat(i.due).toFixed(2)}</td><td style="display:flex; gap:8px;"><button class="btn btn-outline" style="padding:6px 12px; font-size:0.9rem;" onclick="printPosFromData('${i.invId}')"><i class="fas fa-print"></i></button> <button class="btn btn-danger admin-only" style="padding:6px 12px; font-size:0.9rem;" onclick='deleteInvoice("${d.id}", ${JSON.stringify(i).replace(/'/g, "\\'")}, "${i.invId}")'><i class="fas fa-trash"></i></button></td></tr>`; }
            if (i.labStatus !== 'تم التسليم') { tbLab += `<tr><td class="en-num">${i.invId}</td><td style="font-weight:700;">${i.pName}</td><td>${i.prodName}</td><td><select onchange="updateLabStatus('${d.id}', this.value)" class="form-control" style="padding:6px; font-size:0.9rem;"><option value="انتظار" ${i.labStatus==='انتظار'?'selected':''}>انتظار</option><option value="جاهز" ${i.labStatus==='جاهز'?'selected':''}>جاهز</option><option value="تم التسليم">تسليم</option></select></td></tr>`; }
        ...
        let tbUnifiedHTML = ""; patientNames.forEach(pName => { tbUnifiedHTML += `<tr><td style="font-weight:700; color:var(--text-dark); font-size:1rem;">${pName}</td><td class="en-num">${uniquePatients[pName].lastVisit?.toLocaleDateString('en-GB')||'--'}</td><td class="en-num" style="font-weight:800; color:var(--brand-primary);">${uniquePatients[pName].totalSpent.toFixed(2)}</td><td><button class="btn btn-outline" style="padding:6px 12px; font-size:0.9rem;" onclick="showPatientHistory('${pName}')"><i class="fas fa-folder-open"></i> السجل</button></td></tr>`; posPatientHtml += `<option value="${pName}">${pName}</option>`; });
    ...
    onSnapshot(collection(db, "users"), (s) => {
        ...
        if (document.getElementById('tb-staff')) {
            document.getElementById('tb-staff').innerHTML = allStaffData.map(d => {
                const statusStr = d.status === 'frozen' ? '<span style="color:var(--danger); font-weight:800;">مجمد</span>' : '<span style="color:var(--success); font-weight:800;">نشط</span>';
                const roleStr = d.role === 'developer' ? 'مطور' : d.role === 'manager' ? 'مدير' : 'موظف';
                return `<tr><td style="font-weight:700;">${d.name}</td><td class="en-num">${d.user}</td><td><span style="background:var(--bg-app); padding:4px 8px; border-radius:4px;">${roleStr}</span></td><td>${statusStr}</td><td style="display:flex; gap:8px;"><button class="btn btn-outline" style="padding:6px 12px; font-size:0.9rem;" onclick="loadStaffForEdit('${d.id}')"><i class="fas fa-edit"></i> تعديل</button> <button class="btn btn-danger" style="padding:6px 12px; font-size:0.9rem;" onclick='deleteUserAccount("${d.id}", "${d.name}", ${JSON.stringify(d).replace(/'/g, "\\'")})'><i class="fas fa-trash"></i></button></td></tr>`;
            }).join('');
        }
    });

    onSnapshot(query(collection(db, "audit_logs"), orderBy("time", "desc"), limit(50)), (s) => {
        if(document.getElementById('tb-secret-audit')) document.getElementById('tb-secret-audit').innerHTML = s.docs.map(d => { const data = d.data(); const dStr = data.time?.toDate().toLocaleString('en-GB') || '--'; return `<tr><td><span style="font-weight:700; color:var(--text-dark);">${data.user}</span></td><td>نظام</td><td style="font-weight:600;">${data.action}</td><td class="en-num">${dStr}</td></tr>`; }).join('');
    });

    onSnapshot(query(collection(db, "recycle_bin"), orderBy("deletedAt", "desc")), (s) => {
        if (document.getElementById('tb-recycle')) document.getElementById('tb-recycle').innerHTML = s.docs.map(d => { const p = d.data(); const dStr = p.deletedAt?.toDate().toLocaleString('en-GB') || '--'; return `<tr><td style="font-weight:700; color:var(--danger);">${p.typeLabel}</td><td>${p.displayName}</td><td><span style="font-weight:600; color:var(--text-muted);">${p.deletedBy}</span></td><td class="en-num">${dStr}</td><td><button class="btn btn-outline" style="padding:6px 12px; font-size:0.9rem;" onclick='universalRestore("${d.id}", "${p.originalCol}", "${p.originalId}", ${JSON.stringify(p.data).replace(/'/g, "\\'")}, "${p.displayName}", "${p.typeLabel}")'><i class="fas fa-undo"></i> استرجاع</button></td></tr>`; }).join(''); 
    });
}
