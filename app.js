...
window.allUnifiedRecords = []; window.allOnlineTests = []; 
let todayInvoicesData = []; let todayExpensesData = []; let allStaffData = [];
...
// ================== المراقبة السرية والتوثيق الدقيق ==================
...
window.universalSoftDelete = async (colName, id, dataObj, displayName, typeLabel) => {
    if (confirm(`هل أنت متأكد من حذف (${displayName})؟ سيتم نقله لسلة المحذوفات الشاملة.`)) {
        try {
...
window.universalRestore = async (recycleId, colName, originalId, dataObj, displayName, typeLabel) => {
    try {
        await setDoc(doc(db, colName, originalId), dataObj); // يرجع بنفس الـ ID الأصلي
...
window.loadStaffForEdit = (id) => {
    ...
    document.getElementById('s-pass').value = ""; // مخفي للأمان
...
window.saveStaff = async () => {
    ...
    if (id) {
        ...
        await updateDoc(doc(db, "users", id), updateData); logAudit(`تعديل صلاحيات حساب: ${name}`); Swal.fire('نجاح', 'تم تحديث الحساب', 'success');
    } else {
        if (!pass) return Swal.fire('خطأ', 'الباسوورد إجباري للحساب الجديد', 'error');
        await addDoc(collection(db, "users"), { name, user, pass, role, status: "active", time: serverTimestamp() }); logAudit(`إنشاء حساب جديد: ${name} بصلاحية ${role}`); Swal.fire('نجاح', 'تم إنشاء الحساب', 'success');
    }
    resetStaffForm();
};
...
window.saveUnifiedRecord = async () => {
    const name = document.getElementById('u-name').value.trim(), phone = document.getElementById('u-phone').value;
...
window.showPatientHistory = (patientName) => {
    ...
    else { records.forEach(r => { tbody.innerHTML += `<tr><td class="en-num">${r.time?.toDate().toLocaleDateString('en-GB') || '--'}</td><td class="en-num">${r.invId}</td><td>${r.prodName}</td><td class="en-num" style="font-weight:bold; color:var(--primary);">${parseFloat(r.total).toFixed(2)}</td><td style="display:flex; gap:5px;"><button class="btn btn-dark" style="padding:5px 10px;" onclick="printFromData('${r.invId}')"><i class="fas fa-print"></i></button> <button class="btn btn-danger admin-only" style="padding:5px 10px;" onclick='deleteInvoice("${r.id}", ${JSON.stringify(r).replace(/'/g, "\\'")}, "${r.invId}")'><i class="fas fa-trash"></i></button></td></tr>`; }); }
    document.getElementById('history-modal').style.display = 'flex'; applyRoles(Auth.user.role);
};
...
window.softDeleteProduct = async (id, data) => { window.universalSoftDelete("products", id, data, data.name, "منتج مخزون"); };
...
window.saveExtCollection = async () => { const id = document.getElementById('ext-col-id').value, name = document.getElementById('ext-col-name').value, type = document.getElementById('ext-col-type').value, img = document.getElementById('ext-col-base64').value; if (!name || !img) return Swal.fire('تنبيه', 'إجباري!', 'warning'); if (id) { await updateDoc(doc(db, "brands", id), { name, type, imageUrl: img }); Swal.fire('تم', 'تم التحديث', 'success'); logAudit(`تعديل ماركة بالمعرض: ${name}`); } else { await addDoc(collection(db, "brands"), { name, type, imageUrl: img, timestamp: serverTimestamp() }); Swal.fire('تم', 'تم الإضافة', 'success'); logAudit(`إضافة ماركة للمعرض: ${name}`);} resetExtColForm(); };
window.deleteExtCollection = async (id, data) => { window.universalSoftDelete("brands", id, data, data.name, "ماركة معرض"); };

window.updateLabStatus = async (id, status) => { await updateDoc(doc(db, "invoices", id), { labStatus: status }); logAudit(`تغيير حالة مختبر إلى: ${status}`);};
...
window.saveCMS = async () => { await setDoc(doc(db, "settings", "cms"), { topbar: document.getElementById('cms-topbar').value, openTime: document.getElementById('cms-open-time').value, closeTime: document.getElementById('cms-close-time').value, statusMode: document.getElementById('cms-status-mode').value }); logAudit(`تحديث إعدادات الموقع الخارجي`); Swal.fire('نجاح', 'تم التحديث', 'success'); };

// ================== المزامنة الحية (Realtime Sync) ==================
function startSync() {
    onSnapshot(query(collection(db, "brands"), orderBy("timestamp", "desc")), (s) => {
        let html = ""; s.forEach(d => { const data = d.data(); const imgSrc = data.imageUrl ? `<img src="${data.imageUrl}" style="width:40px; border-radius:4px;">` : 'بدون صورة'; const typeLabel = data.type === 'sun' ? '<span style="color:var(--warning); font-weight:bold;">شمسي</span>' : '<span style="color:var(--primary); font-weight:bold;">طبي</span>'; html += `<tr><td>${data.name}</td><td>${typeLabel}</td><td><button class="btn btn-warning" style="padding:5px 10px;" onclick='loadExtCollectionForEdit("${d.id}", "${data.name}", "${data.type}", "${data.imageUrl||''}")'><i class="fas fa-edit"></i></button> <button class="btn btn-danger" style="padding:5px 10px;" onclick='deleteExtCollection("${d.id}", ${JSON.stringify(data).replace(/'/g, "\\'")})'><i class="fas fa-trash"></i></button></td></tr>`; });
    ...
    onSnapshot(query(collection(db, "products"), orderBy("time", "desc")), (s) => {
        ...
        s.forEach(d => { const p = d.data(); invHtml += `<tr><td>${p.name}</td><td>${p.type}</td><td class="en-num">${p.qty}</td><td class="en-num">${p.price}</td><td style="display:flex; gap:5px;"><button class="btn btn-warning" style="padding:5px 10px;" onclick='loadProductForEdit("${d.id}", ${JSON.stringify(p).replace(/'/g, "\\'")})'><i class="fas fa-edit"></i></button><button class="btn btn-danger" style="padding:5px 10px;" onclick='softDeleteProduct("${d.id}", ${JSON.stringify(p).replace(/'/g, "\\'")})'><i class="fas fa-trash"></i></button></td></tr>`; if (p.qty > 0) posProdHtml += `<option value="${d.id}" data-price="${p.price}" data-qty="${p.qty}">${p.name}</option>`; });
    ...
    onSnapshot(query(collection(db, "invoices"), orderBy("time", "desc")), (s) => {
        ...
        s.forEach(d => { 
            const i = d.data(); i.id = d.id; todayInvoicesData.push(i); window.allUnifiedRecords.push(i);
            if (!i.isUnified) { tbInvoices += `<tr><td class="en-num">${i.invId}</td><td style="font-weight:bold;">${i.pName}</td><td>${i.prodName}</td><td class="en-num" style="font-weight:bold; color:var(--primary);">${parseFloat(i.total).toFixed(2)}</td><td class="en-num" style="color:var(--danger); font-weight:bold;">${parseFloat(i.due).toFixed(2)}</td><td style="display:flex; gap:5px;"><button class="btn btn-dark" onclick="printPosFromData('${i.invId}')" style="padding:5px 10px;"><i class="fas fa-print"></i></button> <button class="btn btn-danger admin-only" onclick='deleteInvoice("${d.id}", ${JSON.stringify(i).replace(/'/g, "\\'")}, "${i.invId}")' style="padding:5px 10px;"><i class="fas fa-trash"></i></button></td></tr>`; }
            ...
            if (i.isUnified && i.rx) { if (!uniquePatients[i.pName]) { uniquePatients[i.pName] = { lastVisit: i.time?.toDate(), totalSpent: 0 }; } uniquePatients[i.pName].totalSpent += Number(i.total); }
        });
    ...
    // سحب حسابات الموظفين (للمطورين والمدراء)
    onSnapshot(collection(db, "users"), (s) => {
        ...
        if (document.getElementById('tb-staff')) {
            document.getElementById('tb-staff').innerHTML = allStaffData.map(d => {
                const statusBtn = d.status === 'frozen' ? `<button class="btn btn-success" style="padding:5px 10px;" onclick="toggleUserFreeze('${d.id}', 'frozen', '${d.name}')"><i class="fas fa-play"></i> فك التجميد</button>` : `<button class="btn btn-warning" style="padding:5px 10px;" onclick="toggleUserFreeze('${d.id}', 'active', '${d.name}')"><i class="fas fa-pause"></i> تجميد</button>`;
                ...
                return `<tr><td style="font-weight:bold;">${d.name}</td><td class="en-num">${d.user}</td><td>${roleBadge}</td><td>${d.status==='frozen'?'<span style="color:red; font-weight:bold;">مجمد</span>':'<span style="color:green; font-weight:bold;">نشط</span>'}</td><td style="display:flex; gap:5px;"><button class="btn btn-primary" style="padding:5px 10px;" onclick="loadStaffForEdit('${d.id}')"><i class="fas fa-edit"></i></button>${statusBtn}<button class="btn btn-danger" style="padding:5px 10px;" onclick='deleteUserAccount("${d.id}", "${d.name}", ${JSON.stringify(d).replace(/'/g, "\\'")})'><i class="fas fa-trash"></i></button></td></tr>`;
            }).join('');
        }
    });

    onSnapshot(query(collection(db, "audit_logs"), orderBy("time", "desc"), limit(50)), (s) => {
        if(document.getElementById('tb-secret-audit')) document.getElementById('tb-secret-audit').innerHTML = s.docs.map(d => {
            const data = d.data(); const dStr = data.time?.toDate().toLocaleString('ar-EG', {dateStyle:'short', timeStyle:'medium'}) || '--';
            return `<tr><td><span class="badge" style="background:#e2e8f0; color:#334155;">${data.user}</span></td><td>جهاز</td><td style="color:var(--primary); font-weight:bold;">${data.action}</td><td class="en-num">${dStr}</td></tr>`;
        }).join('');
    });
    ...
    onSnapshot(query(collection(db, "recycle_bin"), orderBy("deletedAt", "desc")), (s) => {
        if (document.getElementById('tb-recycle')) document.getElementById('tb-recycle').innerHTML = s.docs.map(d => { 
            ...
            return `<tr><td style="font-weight:bold; color:var(--danger);">${p.typeLabel}</td><td style="font-family:'Tajawal',sans-serif;">${p.displayName}</td><td><span class="badge" style="background:#f1f5f9; color:#475569;">${p.deletedBy}</span></td><td class="en-num">${dStr}</td><td><button class="btn btn-success" style="padding:8px 15px;" onclick='universalRestore("${d.id}", "${p.originalCol}", "${p.originalId}", ${JSON.stringify(p.data).replace(/'/g, "\\'")}, "${p.displayName}", "${p.typeLabel}")'><i class="fas fa-undo"></i> استرجاع</button></td></tr>`; 
        }).join(''); 
    });

    onSnapshot(doc(db, "settings", "cms"), (docSnap) => {
        if (docSnap.exists() && document.getElementById('cms-topbar')) {
            const data = docSnap.data(); document.getElementById('cms-topbar').value = data.topbar || ''; if(document.getElementById('cms-open-time')) document.getElementById('cms-open-time').value = data.openTime || ''; if(document.getElementById('cms-close-time')) document.getElementById('cms-close-time').value = data.closeTime || ''; if(document.getElementById('cms-status-mode')) document.getElementById('cms-status-mode').value = data.statusMode || 'auto';
        }
    });
}
