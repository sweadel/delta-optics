...
// ================== واجهة المستخدم ==================
window.toggleSidebar = () => { const s = document.getElementById('main-sidebar'); const o = document.getElementById('sidebar-overlay'); if(s) s.classList.toggle('active'); if(o) o.classList.toggle('active'); };
window.showView = (id) => { document.querySelectorAll('.view').forEach(v => { v.classList.remove('active'); v.style.display = 'none'; }); document.querySelectorAll('.nav-link').forEach(l => l.classList.remove('active')); const target = document.getElementById(id); if(target) { target.classList.add('active'); target.style.display = 'block'; } if(event && event.currentTarget) event.currentTarget.classList.add('active'); window.scrollTo({ top: 0, behavior: 'smooth' }); };
...
window.handleLogin = async () => {
    const u = document.getElementById('auth-u').value, p = document.getElementById('auth-p').value; const res = await Auth.login(u, p);
    if (res.success) { document.getElementById('login-modal').style.display = 'none'; document.getElementById('display-user').innerText = Auth.user.name; applyRoles(Auth.user.role); logAudit("تسجيل دخول للنظام"); startSync(); window.showView('dash'); } else { Swal.fire('مرفوض', res.msg, 'error'); }
};
...
window.universalSoftDelete = async (colName, id, dataObj, displayName, typeLabel) => {
    if (confirm(`هل أنت متأكد من حذف (${displayName})؟ سيتم نقله لسلة المحذوفات.`)) {
        try { await addDoc(collection(db, "recycle_bin"), { originalCol: colName, originalId: id, data: dataObj, displayName: displayName, typeLabel: typeLabel, deletedBy: Auth.user.name, deletedAt: serverTimestamp() }); await deleteDoc(doc(db, colName, id)); logAudit(`حذف للمهملات: [${typeLabel}] ${displayName}`); Swal.fire({ icon: 'success', title: 'تم الحذف', text: 'تم النقل بنجاح إلى سلة المحذوفات.', timer: 1500, showConfirmButton: false }); } catch (e) { Swal.fire('خطأ', 'مشكلة بالحذف', 'error'); }
    }
};
window.universalRestore = async (recycleId, colName, originalId, dataObj, displayName, typeLabel) => {
    try { await setDoc(doc(db, colName, originalId), dataObj); await deleteDoc(doc(db, "recycle_bin", recycleId)); logAudit(`استرجاع من المهملات: [${typeLabel}] ${displayName}`); Swal.fire({ icon: 'success', title: 'تم الاسترجاع', text: 'عادت البيانات لمكانها الأصلي.', timer: 1500, showConfirmButton: false }); } catch (e) { Swal.fire('خطأ', 'مشكلة بالاسترجاع', 'error'); }
};
...
window.renderDailyLedger = () => {
    ...
    todayInvoicesData.forEach(i => { if (i.time?.toDate().toDateString() === todayStr) { totalSales += Number(i.total || 0); totalPaid += Number(i.paid || 0); totalDues += Number(i.due || 0); combined.push({ timeObj: i.time?.toDate(), timeStr: i.time?.toDate().toLocaleTimeString('en-US', {hour:'2-digit', minute:'2-digit'}) || '--', desc: i.isUnified ? `ملف: ${i.pName} (${i.doctor||'--'})` : `بيع: ${i.pName} (${i.doctor||'--'})`, val: `<span class="en-num" style="color:var(--success); font-weight:bold;">+ ${Number(i.paid||0).toFixed(2)}</span>` }); } });
    todayExpensesData.forEach(e => { if (e.time?.toDate().toDateString() === todayStr) { totalExpenses += Number(e.amount || 0); combined.push({ timeObj: e.time?.toDate(), timeStr: e.time?.toDate().toLocaleTimeString('en-US', {hour:'2-digit', minute:'2-digit'}) || '--', desc: `مصروف: ${e.desc} (${e.user})`, val: `<span class="en-num" style="color:var(--danger); font-weight:bold;">- ${Number(e.amount||0).toFixed(2)}</span>` }); } });
    combined.sort((a,b) => (b.timeObj?.getTime() || 0) - (a.timeObj?.getTime() || 0)); let html = ''; combined.forEach(r => { html += `<tr><td class="en-num">${r.timeStr}</td><td>${r.desc}</td><td>${r.val}</td></tr>`; }); if(combined.length === 0) html = '<tr><td colspan="3" style="text-align:center; color:gray;">لا يوجد حركات اليوم</td></tr>';
...
// ================== العيادة ==================
window.calcUnifiedTotal = () => { const frame = parseFloat(document.getElementById('u-frame-price').value) || 0; const lenses = parseFloat(document.getElementById('u-lenses-price').value) || 0; const cl = parseFloat(document.getElementById('u-cl-price').value) || 0; const extras = parseFloat(document.getElementById('u-extras-price').value) || 0; const subtotal = frame + lenses + cl + extras; document.getElementById('u-subtotal').value = subtotal.toFixed(2); const discountPercent = parseFloat(document.getElementById('u-discount').value) || 0; const finalTotal = subtotal - (subtotal * (discountPercent / 100)); document.getElementById('u-total').value = finalTotal.toFixed(2); const paid = parseFloat(document.getElementById('u-paid').value) || 0; let due = finalTotal - paid; if (due < 0) due = 0; document.getElementById('u-due').value = due.toFixed(2); document.getElementById('u-due-display').innerText = due.toFixed(2); };
window.saveUnifiedRecord = async () => {
    ...
    try { await addDoc(collection(db, "invoices"), { invId, pName: name, phone: phone, prodName: fullProdName, subtotal, discountPercent, total, paid, due, paymentMethod, labStatus: 'انتظار', time: serverTimestamp(), isUnified: true, rx: rxData, detailedSales: salesData, doctor: doctorName }); logAudit(`إصدار ملف عيادة: ${name} (${invId})`); Swal.fire({ icon: 'success', title: 'تم الحفظ', timer: 1500, showConfirmButton: false }); printUnifiedInvoice({ invId, pName: name, phone, time: new Date(), doctor: doctorName, rx: rxData, detailedSales: salesData, subtotal, discountPercent, total, paid, due, paymentMethod }); document.getElementById('u-name').value = ''; document.getElementById('u-phone').value = ''; document.querySelectorAll('.rx-table input, .grid-2 input, .product-row input').forEach(inp => inp.value = ''); document.getElementById('u-discount').value = "0"; document.getElementById('u-paid').value = ''; calcUnifiedTotal(); } catch (e) { console.error(e); }
};
...
function printUnifiedInvoice(data) {
    ...
    document.getElementById('pr-content').innerHTML = `<div class="print-card"><div class="print-header"><h2>Delta Optics</h2><h4>Medical Rx & Receipt</h4></div><div class="print-info-box"><div class="print-info-row"><span><b>No:</b> <span class="en-num-print">${data.invId}</span></span><span><span class="en-num-print">${dateStr} ${timeStr}</span></span></div><div class="print-info-row"><span><b>المراجع:</b> ${data.pName}</span><span class="en-num-print">${data.phone || '---'}</span></div></div><div class="print-section-title">القياسات (Optical Rx)</div><table class="print-table"><tr><th>Eye</th><th>SPH</th><th>CYL</th><th>AXIS</th><th>ADD</th></tr><tr><th>R (OD)</th><td><span class="en-num-print">${rx.od.s||'-'}</span></td><td><span class="en-num-print">${rx.od.c||'-'}</span></td><td><span class="en-num-print">${rx.od.a||'-'}</span></td><td><span class="en-num-print">${rx.od.add||'-'}</span></td></tr><tr><th>L (OS)</th><td><span class="en-num-print">${rx.os.s||'-'}</span></td><td><span class="en-num-print">${rx.os.c||'-'}</span></td><td><span class="en-num-print">${rx.os.a||'-'}</span></td><td><span class="en-num-print">${rx.os.add||'-'}</span></td></tr></table><div style="font-size:9pt; margin-bottom:12px;"><b>PD:</b> <span class="en-num-print">${rx.pd||'-'}</span> | <b>ملاحظة:</b> ${rx.notes||'-'}</div>${data.subtotal > 0 ? `<div class="print-section-title">المشتريات (Purchases)</div><table class="print-table"><tr><th>البيان</th><th style="width:50px;">JOD</th></tr>${s.frame.type ? `<tr><td>إطار: ${s.frame.type}</td><td><span class="en-num-print">${s.frame.price.toFixed(2)}</span></td></tr>` : ''}${s.lenses.type ? `<tr><td>عدسات: ${s.lenses.type}</td><td><span class="en-num-print">${s.lenses.price.toFixed(2)}</span></td></tr>` : ''}${s.cl.type ? `<tr><td>لاصق: ${s.cl.type}</td><td><span class="en-num-print">${s.cl.price.toFixed(2)}</span></td></tr>` : ''}${s.extras.type ? `<tr><td>أخرى: ${s.extras.type}</td><td><span class="en-num-print">${s.extras.price.toFixed(2)}</span></td></tr>` : ''}</table><div class="print-totals-box"><div class="print-total-row"><span>Subtotal:</span><span class="en-num-print">${data.subtotal.toFixed(2)}</span></div>${data.discountPercent > 0 ? `<div class="print-total-row"><span>Discount ${data.discountPercent}%:</span><span class="en-num-print">- ${(data.subtotal * (data.discountPercent/100)).toFixed(2)}</span></div>` : ''}<div class="print-total-row final"><span>Total:</span><span class="en-num-print">${data.total.toFixed(2)}</span></div><div class="print-total-row"><span>Paid (${pMethod}):</span><span class="en-num-print">${data.paid.toFixed(2)}</span></div><div class="print-total-row"><span>Due الباقي:</span><span class="en-num-print">${data.due.toFixed(2)}</span></div></div>` : ''}<div style="text-align:center; margin-top:15px; font-weight:600; font-size:8pt;"><p style="margin:0;">بواسطة: ${data.doctor || 'موظف'} | ✨ نتمنى لكم رؤية واضحة ✨</p></div></div>`;
    window.print();
}
...
window.showPatientHistory = (patientName) => {
    ...
    else { records.forEach(r => { tbody.innerHTML += `<tr><td class="en-num">${r.time?.toDate().toLocaleDateString('en-GB') || '--'}</td><td class="en-num">${r.invId}</td><td>${r.prodName}</td><td class="en-num" style="font-weight:bold; color:var(--primary);">${parseFloat(r.total).toFixed(2)}</td><td style="display:flex; gap:5px;"><button class="btn-pro" style="padding:6px 12px; font-size:0.9rem; background:#f1f5f9; color:var(--text-main);" onclick="printFromData('${r.invId}')"><i class="fas fa-print"></i></button> <button class="btn-pro admin-only" style="padding:6px 12px; font-size:0.9rem; background:#fef2f2; color:var(--danger);" onclick='deleteInvoice("${r.id}", ${JSON.stringify(r).replace(/'/g, "\\'")}, "${r.invId}")'><i class="fas fa-trash"></i></button></td></tr>`; }); }
...
window.printPatientHistory = () => {
    ...
    document.getElementById('pr-content').innerHTML = `<div class="print-card"><div class="print-header"><h2>Delta Optics</h2><h4>Patient History | كشف سجل</h4></div><div class="print-info-box"><b>المراجع:</b> ${pName}<br><b>الطباعة:</b> <span class="en-num-print">${new Date().toLocaleDateString('en-GB')}</span></div><table class="print-table"><tr><th>التاريخ</th><th>الملف</th><th>التفاصيل</th><th>JOD</th></tr>${rowsHtml}</table><div class="print-totals-box">Total Spent: <span class="en-num-print">${totalSpent.toFixed(2)}</span> JOD</div></div>`; window.print(); logAudit(`طباعة كشف حساب للمريض: ${pName}`);
};
...
window.printPosFromData = (invId, pName, prodName, subtotal, discountPercent, total, paid, due, pMethod) => {
    ...
    document.getElementById('pr-content').innerHTML = `<div class="print-card" style="width:100mm; text-align:center;"><h2 style="margin:0; font-family:Arial; font-size:14pt;">Delta Optics</h2><p style="font-size:9pt;">Sales Receipt: <span class="en-num-print">${d.invId}</span></p><p style="font-size:9pt;">Date: <span class="en-num-print">${d.time.toLocaleDateString('en-GB')}</span></p><hr><p style="font-size:9pt;"><b>الزبون:</b> ${d.pName}</p><p style="font-size:9pt;"><b>المنتج:</b> ${d.prodName}</p><hr><h3 style="margin:5px; font-size:12pt;">Total: <span class="en-num-print">${d.total.toFixed(2)}</span> JOD</h3><p style="margin:2px; font-size:9pt;">Paid: <span class="en-num-print">${d.paid.toFixed(2)}</span> | Due: <span class="en-num-print">${d.due.toFixed(2)}</span></p></div>`; window.print(); logAudit(`طباعة إيصال كاشير: ${invId}`);
};
...
window.resetExtColForm = () => { document.getElementById('ext-col-id').value = ""; document.getElementById('ext-col-name').value = ""; document.getElementById('ext-col-type').value = "medical"; document.getElementById('ext-col-base64').value = ""; };
window.loadExtCollectionForEdit = (id, name, type, img) => { document.getElementById('ext-col-id').value = id; document.getElementById('ext-col-name').value = name; document.getElementById('ext-col-type').value = type || "medical"; document.getElementById('ext-col-base64').value = img || ""; window.scrollTo({ top: 0, behavior: 'smooth' }); };
window.saveExtCollection = async () => { const id = document.getElementById('ext-col-id').value, name = document.getElementById('ext-col-name').value, type = document.getElementById('ext-col-type').value, img = document.getElementById('ext-col-base64').value; if (!name || !img) return Swal.fire('تنبيه', 'إجباري!', 'warning'); if (id) { await updateDoc(doc(db, "brands", id), { name, type, imageUrl: img }); Swal.fire('تم', 'تم التحديث', 'success'); logAudit(`تعديل موديل بالمعرض: ${name}`);} else { await addDoc(collection(db, "brands"), { name, type, imageUrl: img, timestamp: serverTimestamp() }); Swal.fire('تم', 'تم الإضافة', 'success'); logAudit(`إضافة موديل للمعرض: ${name}`);} resetExtColForm(); };
...
window.saveCMS = async () => { await setDoc(doc(db, "settings", "cms"), { topbar: document.getElementById('cms-topbar').value, openTime: document.getElementById('cms-open-time').value, closeTime: document.getElementById('cms-close-time').value, statusMode: document.getElementById('cms-status-mode').value }); Swal.fire('نجاح', 'تم النشر', 'success'); };

// ================== المزامنة החية ==================
function startSync() {
    onSnapshot(query(collection(db, "brands"), orderBy("timestamp", "desc")), (s) => {
        let html = ""; s.forEach(d => { const data = d.data(); const typeLabel = data.type === 'sun' ? 'شمسي' : 'طبي'; html += `<tr><td>${data.name}</td><td>${typeLabel}</td><td><button class="btn-pro" style="background:#f1f5f9; color:var(--text-main); padding:6px 12px; font-size:0.9rem;" onclick='loadExtCollectionForEdit("${d.id}", "${data.name}", "${data.type}", "${data.imageUrl||''}")'><i class="fas fa-edit"></i></button> <button class="btn-pro admin-only" style="background:#fef2f2; color:var(--danger); padding:6px 12px; font-size:0.9rem;" onclick='deleteExtCollection("${d.id}", ${JSON.stringify(data).replace(/'/g, "\\'")})'><i class="fas fa-trash"></i></button></td></tr>`; });
    ...
    onSnapshot(query(collection(db, "products"), orderBy("time", "desc")), (s) => {
        ...
        s.forEach(d => { const p = d.data(); invHtml += `<tr><td>${p.name}</td><td>${p.type}</td><td class="en-num">${p.qty}</td><td class="en-num">${p.price}</td><td style="display:flex; gap:5px;"><button class="btn-pro" style="background:#f1f5f9; color:var(--text-main); padding:6px 12px; font-size:0.9rem;" onclick='loadProductForEdit("${d.id}", ${JSON.stringify(p).replace(/'/g, "\\'")})'><i class="fas fa-edit"></i></button><button class="btn-pro admin-only" style="background:#fef2f2; color:var(--danger); padding:6px 12px; font-size:0.9rem;" onclick='softDeleteProduct("${d.id}", ${JSON.stringify(p).replace(/'/g, "\\'")})'><i class="fas fa-trash"></i></button></td></tr>`; if (p.qty > 0) posProdHtml += `<option value="${d.id}" data-price="${p.price}" data-qty="${p.qty}">${p.name}</option>`; });
    ...
    onSnapshot(query(collection(db, "invoices"), orderBy("time", "desc")), (s) => {
        ...
        s.forEach(d => { 
            ...
            if (!i.isUnified) { window.allUnifiedRecords.push(i); tbInvoices += `<tr><td class="en-num">${i.invId}</td><td style="font-weight:bold;">${i.pName}</td><td>${i.prodName}</td><td class="en-num" style="font-weight:bold; color:var(--primary);">${parseFloat(i.total).toFixed(2)}</td><td class="en-num" style="color:var(--danger); font-weight:bold;">${parseFloat(i.due).toFixed(2)}</td><td style="display:flex; gap:5px;"><button class="btn-pro" style="background:#f1f5f9; color:var(--text-main); padding:6px 12px; font-size:0.9rem;" onclick="printPosFromData('${i.invId}')"><i class="fas fa-print"></i></button> <button class="btn-pro admin-only" style="background:#fef2f2; color:var(--danger); padding:6px 12px; font-size:0.9rem;" onclick='deleteInvoice("${d.id}", ${JSON.stringify(i).replace(/'/g, "\\'")}, "${i.invId}")'><i class="fas fa-trash"></i></button></td></tr>`; }
            if (i.labStatus !== 'تم التسليم') { tbLab += `<tr><td class="en-num">${i.invId}</td><td style="font-weight:bold;">${i.pName}</td><td>${i.prodName}</td><td><select onchange="updateLabStatus('${d.id}', this.value)" class="pro-input" style="padding:6px;"><option value="انتظار" ${i.labStatus==='انتظار'?'selected':''}>انتظار</option><option value="جاهز" ${i.labStatus==='جاهز'?'selected':''}>جاهز</option><option value="تم التسليم">تسليم</option></select></td></tr>`; }
        ...
        let tbUnifiedHTML = ""; patientNames.forEach(pName => { tbUnifiedHTML += `<tr><td style="font-weight:bold; color:var(--primary); font-size:1.05rem;">${pName}</td><td class="en-num">${uniquePatients[pName].lastVisit?.toLocaleDateString('en-GB')||'--'}</td><td class="en-num" style="font-weight:bold;">${uniquePatients[pName].totalSpent.toFixed(2)}</td><td><button class="btn-pro btn-primary" style="padding:6px 12px; font-size:0.9rem;" onclick="showPatientHistory('${pName}')">فتح السجل</button></td></tr>`; posPatientHtml += `<option value="${pName}">${pName}</option>`; });
    ...
    onSnapshot(collection(db, "users"), (s) => {
        ...
        if (document.getElementById('tb-staff')) {
            document.getElementById('tb-staff').innerHTML = allStaffData.map(d => {
                const statusBtn = d.status === 'frozen' ? `<button class="btn-pro" style="background:var(--success); color:white; padding:6px 12px; font-size:0.9rem;" onclick="toggleUserFreeze('${d.id}', 'frozen', '${d.name}')"><i class="fas fa-play"></i> فك تجميد</button>` : `<button class="btn-pro" style="background:var(--warning); color:white; padding:6px 12px; font-size:0.9rem;" onclick="toggleUserFreeze('${d.id}', 'active', '${d.name}')"><i class="fas fa-pause"></i> تجميد</button>`;
                ...
                return `<tr><td style="font-weight:bold;">${d.name}</td><td class="en-num">${d.user}</td><td>${roleBadge}</td><td>${d.status==='frozen'?'<span style="color:var(--danger); font-weight:bold;">مجمد</span>':'<span style="color:var(--success); font-weight:bold;">نشط</span>'}</td><td style="display:flex; gap:5px;"><button class="btn-pro" style="background:#f1f5f9; color:var(--text-main); padding:6px 12px; font-size:0.9rem;" onclick="loadStaffForEdit('${d.id}')"><i class="fas fa-edit"></i> تعديل</button>${statusBtn}<button class="btn-pro" style="background:#fef2f2; color:var(--danger); padding:6px 12px; font-size:0.9rem;" onclick='deleteUserAccount("${d.id}", "${d.name}", ${JSON.stringify(d).replace(/'/g, "\\'")})'><i class="fas fa-trash"></i></button></td></tr>`;
            }).join('');
        }
    });

    onSnapshot(query(collection(db, "audit_logs"), orderBy("time", "desc"), limit(50)), (s) => {
        if(document.getElementById('tb-secret-audit')) document.getElementById('tb-secret-audit').innerHTML = s.docs.map(d => { const data = d.data(); const dStr = data.time?.toDate().toLocaleString('en-GB') || '--'; return `<tr><td><span style="font-weight:bold; color:var(--text-main);">${data.user}</span></td><td>جهاز نظام</td><td style="color:var(--primary); font-weight:bold;">${data.action}</td><td class="en-num">${dStr}</td></tr>`; }).join('');
    });

    onSnapshot(query(collection(db, "recycle_bin"), orderBy("deletedAt", "desc")), (s) => {
        if (document.getElementById('tb-recycle')) document.getElementById('tb-recycle').innerHTML = s.docs.map(d => { const p = d.data(); const dStr = p.deletedAt?.toDate().toLocaleString('en-GB') || '--'; return `<tr><td style="font-weight:bold; color:var(--danger);">${p.typeLabel}</td><td>${p.displayName}</td><td><span style="font-weight:bold; color:var(--text-muted);">${p.deletedBy}</span></td><td class="en-num">${dStr}</td><td><button class="btn-pro btn-primary" style="padding:8px 15px; font-size:0.9rem;" onclick='universalRestore("${d.id}", "${p.originalCol}", "${p.originalId}", ${JSON.stringify(p.data).replace(/'/g, "\\'")}, "${p.displayName}", "${p.typeLabel}")'><i class="fas fa-undo"></i> استرجاع</button></td></tr>`; }).join(''); 
    });
}
